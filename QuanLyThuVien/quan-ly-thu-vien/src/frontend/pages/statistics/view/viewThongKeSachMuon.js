...
function ViewThongKeSachMuon() {
  ...
  return (
    <div>
      <Link to="/BaoCaoThongKe"><i className="fas fa-arrow-left iconspacing"></i><button className="btn-trove">Quay lại</button></Link>
      <h3>
        <strong>Thống kê sách được mượn nhiều</strong>
      </h3>
      <hr />
      <div className="row filter-container">
        ...
        <div className="col-3 form-group">
          ...
          <select id="from-year" value={year} onChange={(e) => setYear(e.target.value)}>
        ...
        <div className="col-2">
          <button id="filter-button" onClick={handleFilter} className="mr-sm-2 mr-md-3">
            Thống kê
          </button>
        </div>
      </div>
      ...
      <p className="p-text" id="filter-info">
        {fromMonth && toMonth && year
          ? `Thống kê sách mượn từ tháng ${fromMonth} đến tháng ${toMonth} năm ${year}`
          ...
          <Bar
            options={{
              ...
              plugins: {
                ...
                title: {
                  display: true,
                  //text: `Thống kê sách mượn từ tháng ${fromMonth} đến tháng ${toMonth}, năm ${year}`,
                },
              },
            }}
            data={chartData}
